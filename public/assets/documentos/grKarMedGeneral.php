<?php
	...
	include "../includes/functions.php";	
		...
		$pdf->Cell(180,8,strtoupper("KARDEX"),0,0,'L');
		$pdf->Ln(1);
		#NOMBRE DEL MEDICAMENTO
		$pdf->SetFont('Arial','B',10);
		$pdf->SetTextColor(10,115,6);		
		$pdf->Cell(180,15,'KARDEX GENERAL',0,0,'L');
		...
		$constock = mysqli_query($conexion, "SELECT DISTINCT k.idmedicamento, k.fechavencimiento,
		m.nombre, m.idforma, m.concentracion, m.idunidad, 
		f.nombre AS desforma, u.abreviatura AS desunidad
		FROM kardex k
		INNER JOIN medicamento m ON k.idmedicamento = m.idmedicamento
		INNER JOIN forma f ON m.idforma = f.idforma
		...
		# SI EXISTEN DATOS CONTINUAMOS EXTRANDO LA INFORMACION DE LOS MOVIMIENTOS 
			...
			$nombre = $datastock['nombre']. ' '.$datastock['desforma']. ' '.$datastock['concentracion']. ''.$datastock['desunidad'];		
			$idmedicamento = $datastock['idmedicamento'];
			...
			$pdf->Cell(150,8,"NO SE REGISTRA MOVIMIENTO DE INGRESO  SALIDA DE MEDICAMENTOS",1,0,'C',true);			
			$pdf->Ln(8);
			$pdf->SetTextColor(39,39,51);
		}
