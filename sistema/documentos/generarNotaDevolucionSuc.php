<?php
	...
	if(empty($_REQUEST['so']) || empty($_REQUEST['idv']))
	{
		echo "No es posible generar la nota de ingreso.";
	}else{

		#Optenemos los datos requeridos para el reporte = NOTA DE INGRESO
		$codsucorigen = $_REQUEST['so'];
		$notadevolucion = $_REQUEST['idv'];	
		$consulta = mysqli_query($conexion, "SELECT * FROM configuracion");
		$resultado = mysqli_fetch_assoc($consulta);
		$lotes = mysqli_query($conexion, "SELECT * FROM devolucion_suc WHERE nodevsuc = $notadevolucion");
		$result_lote = mysqli_fetch_assoc($lotes);
		$proveedores = mysqli_query($conexion, "SELECT * FROM sucursal WHERE idsucursal = $codsucorigen");
		$result_proveedor = mysqli_fetch_assoc($proveedores);		
		$medicamentos = mysqli_query($conexion, "SELECT ds. nodevsuc, ds.idmedicamento, ds.cantidad, ds.fechavencimiento,		
														m.idmedicamento, m.nombre, m.preciounitario
														FROM detalledevolucionsuc  ds
														INNER JOIN medicamento m ON ds.nodevsuc = $notadevolucion 
		...
		$pdf->Cell(180,10,utf8_decode(strtoupper("NOTA DE DEVOLUCIÓN")),0,0,'C');
		$pdf->Ln(20);
		...
		$pdf->Cell(15, 5, utf8_decode("DETALLE DEVOLUCIÓN: "), 0, 0, 'L');
		$pdf->Ln(7);		
		#FECHA
		$pdf->SetFont('Arial','',10);
		$pdf->SetTextColor(39,39,51);
		$pdf->Cell(45,7,utf8_decode("Fecha devolución:"),0,0);
		$pdf->SetTextColor(97,97,97);
		$pdf->Cell(100,7,utf8_decode($result_lote['fecha']),0,0,'L');		
		$pdf->SetFont('Arial','B',10);
		$pdf->SetTextColor(39,39,51);
		$pdf->Cell(35,7,utf8_decode(strtoupper("T / Devolucion Nro.")),0,0,'C');
		$pdf->Ln(5);
		...
		$pdf->SetTextColor(97,97,97);
		$pdf->Cell(35,7,$notadevolucion,0,0,'C');
		$pdf->Ln(5);
		#DESCRIPCION 
		$pdf->SetFont('Arial','',10);
		$pdf->SetTextColor(39,39,51);
		...
		$pdf->SetTextColor(97,97,97);
		$pdf->Cell(100,7,utf8_decode($result_lote['notadevolucion']),0,0,'L');		
		$pdf->Ln(10);

		#Datos del Proveedor
		$pdf->SetFont('Arial','B',10);
		$pdf->Cell(15, 5, "DATOS SUCURSAL ORIGEN: ", 0, 0, 'L');
		$pdf->Ln(7);		
		#NOMBRE - NIT 
		$pdf->SetFont('Arial','',10);
		$pdf->SetTextColor(39,39,51);
		...
		$pdf->SetTextColor(97,97,97);
		$pdf->Cell(90,7,utf8_decode($result_proveedor['nombre']),0,0,'L');
		$pdf->SetTextColor(39,39,51);
		$pdf->Cell(25,7,utf8_decode("CIUDAD: "),0,0,'L');
		$pdf->SetTextColor(97,97,97);
		$pdf->Cell(40,7,$result_proveedor['ciudad'],0,0,'L');
		$pdf->SetTextColor(39,39,51);
		$pdf->Ln(5);
		#DIRECCION - TELEFONO
		$pdf->SetTextColor(39,39,51);
		...
		$pdf->SetTextColor(97,97,97);
		$pdf->Cell(90,7,$result_proveedor['direccion'],0,0);
		$pdf->SetTextColor(39,39,51);				
		...
		$pdf->Output("I","Nota_Devolucion_Nro.$notadevolucion.pdf",true);	
